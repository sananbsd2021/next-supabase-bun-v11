...
export default function EdunewsPage() {
  ...
  useEffect(() => {
    async function fetchEdunews() {
      try {
        const response = await fetch('/api/edunews'); // ตรวจสอบให้แน่ใจว่า endpoint ถูกต้อง
        if (!response.ok) {
  ...
  const handleAddEdunews = async (data: { title: string; description: string; imageUrl: string }) => {
